...
func (s *SysUserService) Add(addDto dto.SysUserAddDto) error {
	...
	var count int64
	orm.DB.Model(&model.SysUser{}).Where("user_name = ?", addDto.UserName).Count(&count)
	if count != 0 {
		return errors.New("账号名称已存在")
	}

	//密码盐
	var salt = utils.GetRoundNumber(15)

	//加密 => MD5（密码+密码盐）
	var pwd = utils.MD5(addDto.Password + salt)
...
func (s *SysUserService) Update(updateDto dto.SysUserUpdateDto) error {
	err := orm.DB.Model(&model.SysUser{}).Where("id = ?", updateDto.Id).Updates(map[string]interface{}{
...
func (s *SysUserService) Delete(id int64) error {
	err := orm.DB.Delete(&model.SysUser{}, "id = ?", id).Error
	return err
}

// Detail 获取用户详情
func (s *SysUserService) Detail(id int64) (obj *dto.SysUserVo, err error) {
	err = orm.DB.Model(&model.SysUser{}).Where("id = ?", id).Scan(&obj).Error
	if err != nil {
		return nil, err
	}
	var roleIds []int64
	err = orm.DB.Model(&model.SysUserRole{}).Select("role_id").Where("user_id = ?", id).Scan(&roleIds).Error
	if err != nil {
		return nil, err
	}
	obj.RoleIds = roleIds
...
func (s *SysUserService) List() (objs []dto.SysUserVo, err error) {
	err = orm.DB.Model(&model.SysUser{}).Scan(&objs).Error
	return objs, err
}
...
func (s *SysUserService) ResetPwd(reqDto dto.ResetPwdDto) error {
	...
	//加密 => MD5（密码+密码盐）
	pwd := utils.MD5(reqDto.Password + salt)

	//更新密码
	err := orm.DB.Model(&model.SysUser{}).Where("id = ?", reqDto.UserId).Updates(map[string]interface{}{
		"salt":     salt,
		"password": pwd,
	}).Error
	return err
...
func SetUserRole(userId int64, roleIds []int64) error {
	...
	err := orm.DB.Delete(&model.SysUserRole{}, "user_id = ?", userId).Error
	if err != nil {
		return err
	}
	if len(roleIds) == 0 {
	...
	err = orm.DB.Create(userRoles).Error

	//删除缓存
	userIdStr := strconv.FormatInt(userId, 10)
	cache.Redis.Del(cache.CacheKeySysUserMenu + userIdStr)
	cache.Redis.Del(cache.CacheKeySysUserPermission + userIdStr)
...
func (s *SysUserService) SetUserStatus(reqDto dto.SetUserStateDto) error {
	...
	//用户状态
	err := orm.DB.Model(&model.SysUser{}).Where("id = ?", reqDto.UserId).Update("status", reqDto.Status).Error
	return err
